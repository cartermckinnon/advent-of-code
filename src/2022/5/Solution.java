...
public class Solution {
    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            System.err.println("usage: Solution INPUT_FILE");
            System.exit(1);
        }
        try (Scanner scanner = new Scanner(new File(args[0]))) {
            ...
            while(scanner.hasNextLine()) {
                ...
                for(int i = 0; i < n; i++) {
                    stacks.get(destStack).addTopCrate(stacks.get(sourceStack).pollTopCrate());;
                }
            }
    ...
    public static Map<String, Stack> parseStacks(List<String> lines) {
        Map<String, Stack> stacks = new TreeMap<>(); // TreeMap so the stacks can be iterated in order of their stackId
    ...
    public static class Stack {
        ...
        public void addCrate(String line) {
